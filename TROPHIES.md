...
- `lj_opt_narrow` moves an unguarded num->int conversion inside an
  ...
  an integer. Not yet reported upstream. Reproducers:
  `tests/reproducers/narrow_toint_fractional.lua` (`string.sub`) and
  `tests/reproducers/narrow_tobit_fractional.lua` (`bit.tobit`).
  ...
  Neither side is a rounding-mode difference: the interpreter and
  the trace use the same primitive (`lj_num2int` / `CONV int.num`
  truncate, `lj_num2bit` / IR `TOBIT` round to nearest). Only the
  order of the conversion and the addition changes.
