...
-- We have snap_stack of type array<array<BV>>, where first index is a number of snapshot,
-- second index is snapshot values
--
-- Example:
-- SNAP #2 [ ---- 0003 0004 ---- ---- 1.42 ]
-- Will be passed in input as [{1, ssa, 3}, {2, ssa, 4}, {5, const, "bv1068876431"} }
-- And this function will return [(1, "<smt formula>"), (2, "<smt formula>"), (5, "<smt formula>")]
--
-- More about snapshot representaion can be found here: https://ujit.readthedocs.io/en/latest/public/tut-snap.html
-- @a snapshot output of ir_dump. In format: array<(slot, value, optional_value)>
local function snap_to_smt_lib(ctx, snapshot)
    ...
    for _, pair in ipairs(snapshot) do
        ...
        elseif value_type == "const" then
            local cnst = string.format('((_ to_fp 11 53) (_ %s 64))', value_data)
            cnst = cnst:gsub('+', '')
