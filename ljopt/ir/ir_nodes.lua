...
-- Extract the SSA reference number from a raw {type, value} irins operand.
-- Returns the integer SSA ref when the operand is an SSA reference, else nil.
local function raw_ssa_ref(raw_op)
    if raw_op ~= nil and raw_op.type == OpKind.SSA then
        return raw_op.value
    end
    return nil
end
...
local function get_nyi_nodes(nodes)
    ...
    for i = 1, table.getn(nodes) do
        ...
        if node == nil or in_loop then
            ...
            -- Check whether either SSA operand depends on a NYI node.
            ...
            if (dep1 and nyi_nodes[dep1]) or (dep2 and nyi_nodes[dep2]) then
                ...
                -- Convert raw tables to OpKind for is_implemented checks so
                -- implementations can inspect operand kinds cleanly.
                local left_op  = OpKind.from_raw(lua_node.op1, lua_node.op1_txt)
                local right_op = OpKind.from_raw(lua_node.op2, lua_node.op2_txt)
