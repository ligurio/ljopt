...
local lj_optimized = ("jit.opt.start(3, 'hotloop=1', 'hotexit=1'%s)"):format(
    ljopt_config.is_narrowing() and '' or ", '-narrow'"
)
...
local function translate(trace_record, ctx_src,
    ...
    trace_record.trace, trace_record.snapshots, unroll_smt =
        loop_unrolling.loop_unrooling_transform(
            ...
            trace_record.linktype, ctx_src.op_stack
        )
    smtlib_buf = smtlib_buf .. (unroll_smt or '')
