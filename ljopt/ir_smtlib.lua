...
local function snapshots2smt(snapshots1, snapshots2, stack1, stack2)
    ...
    if ljopt_config.is_verify_correctness() then
        return ''
    end
    ...
    smt_result = smt_result .. ([[    (and (< witness_ptr %d)
         (not (= (select (select %s %s) witness_ptr)
                 (select (select %s %s) witness_ptr))))
]]):format(
        smt_constants.LOCAL_SLOT_BASE,
        stack1._name, stack1:final_version(),
        stack2._name, stack2:final_version()
    )
