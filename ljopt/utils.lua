...
-- We need to add to every snapshot number of
-- instructions with guard. The function sorts
-- snapshots by nins, and walks over a trace
-- adding guard to the last snapshot.
    ...
    for ir_id, ir_node in pairs(trace_record.trace) do
        if (ir_node ~= nil and ir_node.irt_guard) then
            ...
            if (ins2snap[cur_snap_id].nins <= ir_id) then
                ...
                if (is_debug) then
                    io.stdout:write(
                        ("Snapshot %d depends on %d"):format(
