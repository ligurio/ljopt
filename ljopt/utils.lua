...
local function print_report(n_passed, n_total, failed_list, timeout_list,
                            start_time)
    io.stdout:write("\n")
    local passed_pct = math.floor(100 * n_passed / n_total)
    io.stdout:write(
        ("%d%% traces passed, %d traces failed, %d timed out out of %d"):
        format(passed_pct, #failed_list, #timeout_list, n_total)
    )
    io.stdout:write("\n")
...
local function verify_traces(solver, traces, trace_locs, checked,
    ...
    print_report(n_passed, #checked, failed_list, timeout_list, start_time)
    if #failed_list > 0 then
        return exit_codes.ERR_VERIFICATION_FAILED
    elseif #timeout_list > 0 then
