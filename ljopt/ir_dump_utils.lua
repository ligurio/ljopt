...
local function inspect_stack_arguments(func, _pc)
...
local function ljopt_record_trace(traceno, func, pc, depth, _callee)
  ...
  -- -- Do not print fmtfunc result, it contains pointer
  -- -- which is not stable.
  -- luacheck: push no max_comment_line_length
  -- if pc <= 0 then
  --   line = sub(line, 1, -2).."         ; ".. fmtfunc(func).. "\n"
  -- end
  -- luacheck: pop
...
local function ljopt_savesnap(tr, nins, snap, snapno, _linktype)
  ...
  for s=0,snap[1]-1 do
    ...
    if shr(sn, 24) == s then
      ...
      if ref < 0 then
        -- Type 1: Constant.
        local const_str, const_tab = ljopt_formatsmt(tr, ref, sn)
        if is_supported then
          table.insert(snapshot, {s, "const", const_tab})
        end
...
local function ljopt_savetrace(tr, ins, flags, irtype, op, op1, op2, op1_tab, op2_tab)
  local tr_id = get_trace_id(tr)
  if tr_id == nil then
    -- Skip this trace.
    return
  ...
  local irins = {
    ...
    irtype = trim(irtype),
    irop = trim(op),
    -- op1 / op2: structured {type, value} tables for the operands (or nil).
    op1 = op1_tab,
    op2 = op2_tab,
    -- op1_txt / op2_txt: raw display strings for the operands (trimmed).
    -- These are needed for literal-mode operands (field names, mode flags,
    -- slot numbers with '#' prefix, etc.) that have no constant table.
    op1_txt = trim(op1),
    op2_txt = trim(op2),
  }
